...
from custom_components.presence_based_lighting import SERVICE_ACQUIRE_CONTROL
from custom_components.presence_based_lighting import SERVICE_DISPATCH_CONTROL
from custom_components.presence_based_lighting import SERVICE_PAUSE_AUTOMATION
from custom_components.presence_based_lighting import SERVICE_RELEASE_CONTROL
from custom_components.presence_based_lighting import SERVICE_RESUME_AUTOMATION
from custom_components.presence_based_lighting import SERVICE_SET_AUTOMATION_STATE
from custom_components.presence_based_lighting.const import (
    AUTOMATION_CONTROL_STATE_ACTIVE,
)
from custom_components.presence_based_lighting.const import AUTOMATION_CONTROL_STATE_OFF
from custom_components.presence_based_lighting.const import AUTOMATION_CONTROL_STATE_ON
from custom_components.presence_based_lighting.const import (
...
class TestServiceRegistration:
    ...
    async def test_services_are_registered(self, mock_hass):
        ...
        assert (DOMAIN, SERVICE_ACQUIRE_CONTROL) in registered_services
        assert (DOMAIN, SERVICE_DISPATCH_CONTROL) in registered_services
        assert (DOMAIN, SERVICE_RELEASE_CONTROL) in registered_services
...
class TestResumeAutomationService:
    ...
    @pytest.mark.asyncio
    async def test_resume_automation_resets_paused_flag(
    ...
    @pytest.mark.asyncio
    async def test_resume_automation_allows_presence_control(
        self, mock_hass, mock_config_entry
    ):
    ...
    @pytest.mark.asyncio
    async def test_resume_specific_entity_only(
...
class TestPauseAutomationService:
    ...
    @pytest.mark.asyncio
    async def test_pause_automation_sets_paused_flag(
        self, mock_hass, mock_config_entry
    ):
    ...
    @pytest.mark.asyncio
    async def test_pause_automation_prevents_presence_control(
        self, mock_hass, mock_config_entry
    ):
...
class TestServiceHandlerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_service_handler_finds_coordinator(
        self, mock_hass, mock_config_entry
    ):
        ...
        room_name = (
            mock_config_entry.data.get(CONF_ROOM_NAME, "").lower().replace(" ", "_")
        )
    ...
    @pytest.mark.asyncio
    async def test_resume_after_manual_off_allows_presence_control(
        self, mock_hass, mock_config_entry
    ):
