...
async def test_living_room_isolated_presence_detection(mock_hass):
    ...
    event_data = _presence_event(
        mock_hass, "binary_sensor.living_room_motion", STATE_ON
    )
...
async def test_living_room_isolated_presence_clear(mock_hass):
    ...
    # Clear presence
    event_data = _presence_event(
        mock_hass, "binary_sensor.living_room_motion", STATE_OFF
    )
    await coordinator._handle_presence_change(MagicMock(data=event_data))
    ...
    assert_service_called(
        mock_hass, "media_player", "turn_off", "media_player.living_room_tv"
    )
...
async def test_all_three_entries_concurrent_presence(mock_hass):
    ...
    tv_on_calls = [
        c
        for c in mock_hass.services.calls
        if c["domain"] == "media_player" and c["service"] == "turn_on"
    ]
    assert len(tv_on_calls) == 0
