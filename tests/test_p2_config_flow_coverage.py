...
from custom_components.presence_based_lighting.config_flow import _get_entity_domain
from custom_components.presence_based_lighting.config_flow import _get_entity_name
from custom_components.presence_based_lighting.config_flow import (
...
def test_create_entry_payload_preserves_activation_catchup_mode():
    ...
    assert (
        payload[CONF_ACTIVATION_CATCHUP_MODE] == ACTIVATION_CATCHUP_CLEARING_AUTHORITY
    )
...
class TestFormatActionOptionLabel:
    def test_with_metadata(self):
        label = _format_action_option_label(
            "turn_on",
            {"icon": "mdi:lamp", "name": "Turn On", "description": "Turn on the light"},
        )
...
class TestConfigFlowManageEntities:
    ...
    async def test_no_action_empty_entities(self):
        ...
        handler._base_data = {
            CONF_ROOM_NAME: "R",
            CONF_PRESENCE_SENSORS: ["binary_sensor.m"],
            CONF_OFF_DELAY: 5,
        }
        handler._controlled_entities = []
...
class TestConfigFlowConfigureEntity:
    ...
    @patch(
        "custom_components.presence_based_lighting.config_flow._get_services_for_entity",
        side_effect=ServiceOptionsUnavailable("nope"),
    )
    ...
    async def test_submit_with_rlc_and_delay(self, _mock):
        ...
        assert len(handler._controlled_entities) == 1
        assert (
            handler._controlled_entities[0].get(CONF_RLC_TRACKING_ENTITY)
            == "sensor.rlc_x"
        )
        assert handler._controlled_entities[0].get(CONF_ENTITY_OFF_DELAY) == 15
        assert (
            handler._controlled_entities[0][CONF_PRESENCE_DETECTED_BRIGHTNESS_PCT] == 75
        )
        assert handler._controlled_entities[0][CONF_PRESENCE_DETECTED_TRANSITION] == 2.5
        assert handler._controlled_entities[0][CONF_PRESENCE_CLEARED_TRANSITION] == 3.5
...
class TestConfigureEntityWithRLC:
    ...
    @patch(
        "custom_components.presence_based_lighting.config_flow.is_rlc_integration_available",
        return_value=True,
    )
    @patch(
        "custom_components.presence_based_lighting.config_flow.get_rlc_sensors_for_entity",
        return_value=["sensor.x_rlc"],
    )
    @patch(
        "custom_components.presence_based_lighting.config_flow.get_all_rlc_sensors",
        return_value=["sensor.x_rlc", "sensor.y_rlc"],
    )
...
class TestOptionsFlowAsyncOptionsFlow:
    ...
    async def test_static_method(self):
        ...
        flow = PresenceBasedLightingFlowHandler.async_get_options_flow(
            _make_config_entry()
        )
...
class TestStateFieldDefaults:
    def test_value_in_ha_values(self):
        result = _state_field_defaults(
            "on", ha_values={"on", "off"}, ui_state={}, ui_key="k"
        )
    ...
    def test_value_not_in_ha_values(self):
        result = _state_field_defaults(
            "custom_val", ha_values={"on", "off"}, ui_state={}, ui_key="k"
        )
    ...
    def test_ui_key_present(self):
        result = _state_field_defaults(
            "on", ha_values={"on"}, ui_state={"k": "prev"}, ui_key="k"
        )
        assert result.selector_default == STATE_OPTION_CUSTOM
