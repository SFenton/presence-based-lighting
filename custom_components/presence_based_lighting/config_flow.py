...
async def _get_services_for_entity(
    ...
    for service_name in sorted(available_services):
        label = _format_action_option_label(
            service_name, metadata_for_domain.get(service_name)
        )
        options.append(
...
def _carry_forward_entity_settings(
    updated_config: dict, current_config: dict | None
) -> dict:
...
def _presence_switch_unique_id(
    entry_id: str | None, entity_id: str | None
) -> str | None:
...
async def _build_state_option_dicts(
    ...
    return StateOptionResult(
        options=options, from_hass=bool(ha_candidates), ha_values=set(ha_candidates)
    )
...
class PresenceBasedLightingFlowHandler(
    _EntityManagementMixin, config_entries.ConfigFlow, domain=DOMAIN
):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            self._base_data = {
                ...
                CONF_ACTIVATION_CONDITIONS: user_input.get(
                    CONF_ACTIVATION_CONDITIONS, []
                ),
                ...
                CONF_FILE_LOGGING_ENABLED: user_input.get(
                    CONF_FILE_LOGGING_ENABLED, DEFAULT_FILE_LOGGING_ENABLED
                ),
                CONF_OFF_DELAY: user_input[CONF_OFF_DELAY],
                CONF_AUTO_REENABLE_PRESENCE_SENSORS: user_input.get(
                    CONF_AUTO_REENABLE_PRESENCE_SENSORS, []
                ),
                CONF_AUTO_REENABLE_VACANCY_THRESHOLD: user_input.get(
                    CONF_AUTO_REENABLE_VACANCY_THRESHOLD,
                    DEFAULT_AUTO_REENABLE_VACANCY_THRESHOLD,
                ),
                CONF_AUTO_REENABLE_START_TIME: user_input.get(
                    CONF_AUTO_REENABLE_START_TIME, DEFAULT_AUTO_REENABLE_START_TIME
                ),
                CONF_AUTO_REENABLE_END_TIME: user_input.get(
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_OFF_DELAY, default=DEFAULT_OFF_DELAY): vol.All(
                    ...
                    vol.Optional(
                        CONF_AUTO_REENABLE_VACANCY_THRESHOLD,
                        default=DEFAULT_AUTO_REENABLE_VACANCY_THRESHOLD,
                    ): vol.All(vol.Coerce(int), vol.Range(min=0, max=100)),
                    vol.Optional(
                        CONF_AUTO_REENABLE_START_TIME,
                        default=DEFAULT_AUTO_REENABLE_START_TIME,
                    ): selector.TimeSelector(),
                    vol.Optional(
                        CONF_AUTO_REENABLE_END_TIME,
                        default=DEFAULT_AUTO_REENABLE_END_TIME,
                    ): selector.TimeSelector(),
                }
            ),
            errors=self._errors,
    ...
    async def async_step_configure_entity(self, user_input=None):
        ...
        defaults.setdefault(CONF_CONTROL_LEASE_MODE, DEFAULT_CONTROL_LEASE_MODE)
        defaults.setdefault(
            CONF_CONTROL_LEASE_BLOCKERS,
        ...
        if entity_delay_default is not None:
            delay_field = vol.Optional(
                CONF_ENTITY_OFF_DELAY, default=entity_delay_default
            )
        ...
        use_dropdown = bool(
            state_option_source.from_hass and state_option_source.options
        )

        if user_input is not None:
            entity_off_delay = user_input.get(CONF_ENTITY_OFF_DELAY)
            resolved_detected_state = str(
                user_input[CONF_PRESENCE_DETECTED_STATE]
            ).strip()
            resolved_cleared_state = str(
                user_input[CONF_PRESENCE_CLEARED_STATE]
            ).strip()
            ...
            if not self._errors:
                ...
                use_interceptor = user_input.get(
                ...
                updated_config = {
                    ...
                    CONF_PRESENCE_DETECTED_SERVICE: user_input[
                        CONF_PRESENCE_DETECTED_SERVICE
                    ],
                    CONF_PRESENCE_DETECTED_STATE: resolved_detected_state,
                    CONF_PRESENCE_CLEARED_SERVICE: user_input[
                        CONF_PRESENCE_CLEARED_SERVICE
                    ],
                    CONF_PRESENCE_CLEARED_STATE: resolved_cleared_state,
                    CONF_RESPECTS_PRESENCE_ALLOWED: user_input[
                        CONF_RESPECTS_PRESENCE_ALLOWED
                    ],
                ...
                if _get_entity_domain(entity_id) == "light":
                    updated_config[CONF_CONTROL_LEASE_MODE] = user_input.get(
                        CONF_CONTROL_LEASE_MODE,
                        defaults[CONF_CONTROL_LEASE_MODE],
                    )
                    updated_config[CONF_CONTROL_LEASE_BLOCKERS] = user_input.get(
                        CONF_CONTROL_LEASE_BLOCKERS,
                        defaults[CONF_CONTROL_LEASE_BLOCKERS],
                    )
                    updated_config[CONF_CONTROL_LEASE_CORRECT_LATE_ON] = user_input.get(
                        CONF_CONTROL_LEASE_CORRECT_LATE_ON,
                        defaults[CONF_CONTROL_LEASE_CORRECT_LATE_ON],
                    )
                ...
                _carry_forward_entity_settings(
                    updated_config, self._current_entity_config
                )
                ...
                self._custom_state_ui = {}
                if getattr(self, "_finalize_after_configure", False) and hasattr(
                    self, "_finalize_and_reload"
                ):
        ...
        if use_dropdown:
            ...
            state_options.append(
                selector.SelectOptionDict(value=STATE_OPTION_CUSTOM, label=CUSTOM_LABEL)
            )
            ...
            detected_defaults = StateFieldDefaults(
                defaults[CONF_PRESENCE_DETECTED_STATE], None
            )
            cleared_defaults = StateFieldDefaults(
                defaults[CONF_PRESENCE_CLEARED_STATE], None
            )
            detected_default = detected_defaults.selector_default
            cleared_default = cleared_defaults.selector_default
        ...
        if _get_entity_domain(entity_id) == "light":
            schema_fields[
                vol.Optional(
                    CONF_CONTROL_LEASE_MODE,
                    default=defaults[CONF_CONTROL_LEASE_MODE],
                )
            ] = selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=[
                        selector.SelectOptionDict(
                            value=CONTROL_LEASE_MODE_OFF,
                            label="Off",
                        ),
                        selector.SelectOptionDict(
                            value=CONTROL_LEASE_MODE_OBSERVE,
                            label="Observe only",
                        ),
                        selector.SelectOptionDict(
                            value=CONTROL_LEASE_MODE_ENFORCE,
                            label="Enforce",
                        ),
                    ],
                    mode=selector.SelectSelectorMode.DROPDOWN,
            ...
            schema_fields[
                vol.Optional(
                    CONF_CONTROL_LEASE_BLOCKERS,
                    default=defaults[CONF_CONTROL_LEASE_BLOCKERS],
                )
            ] = selector.EntitySelector(
                selector.EntitySelectorConfig(
            ...
            schema_fields[
                vol.Optional(
                    CONF_CONTROL_LEASE_CORRECT_LATE_ON,
                    default=defaults[CONF_CONTROL_LEASE_CORRECT_LATE_ON],
                )
            ] = selector.BooleanSelector()
            schema_fields[
                vol.Optional(
    ...
    def _create_entry_payload(self) -> dict:
        return {
            ...
            CONF_CLEARING_SENSORS_AUTO_DISCOVERED: self._base_data.get(
                CONF_CLEARING_SENSORS_AUTO_DISCOVERED, False
            ),
            CONF_ACTIVATION_CONDITIONS: self._base_data.get(
                CONF_ACTIVATION_CONDITIONS, []
            ),
            ...
            CONF_FILE_LOGGING_ENABLED: self._base_data.get(
                CONF_FILE_LOGGING_ENABLED, DEFAULT_FILE_LOGGING_ENABLED
            ),
            CONF_OFF_DELAY: self._base_data.get(CONF_OFF_DELAY, DEFAULT_OFF_DELAY),
            CONF_CONTROLLED_ENTITIES: self._controlled_entities,
            CONF_AUTO_REENABLE_PRESENCE_SENSORS: self._base_data.get(
                CONF_AUTO_REENABLE_PRESENCE_SENSORS, []
            ),
            CONF_AUTO_REENABLE_VACANCY_THRESHOLD: self._base_data.get(
                CONF_AUTO_REENABLE_VACANCY_THRESHOLD,
                DEFAULT_AUTO_REENABLE_VACANCY_THRESHOLD,
            ),
            CONF_AUTO_REENABLE_START_TIME: self._base_data.get(
                CONF_AUTO_REENABLE_START_TIME, DEFAULT_AUTO_REENABLE_START_TIME
            ),
            CONF_AUTO_REENABLE_END_TIME: self._base_data.get(
                CONF_AUTO_REENABLE_END_TIME, DEFAULT_AUTO_REENABLE_END_TIME
            ),
        }
    ...
    async def async_step_manage_entities(self, user_input=None):
        ...
        options = [
            selector.SelectOptionDict(
                value=ACTION_NO_ACTION, label="Submit pending changes"
            ),
            ...
            selector.SelectOptionDict(
                value=ACTION_DELETE_ENTITIES, label="Delete entities"
            ),
        ]
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        FIELD_LANDING_ACTION, default=ACTION_NO_ACTION
                    ): selector.SelectSelector(
                        selector.SelectSelectorConfig(
                            options=options,
                            mode=selector.SelectSelectorMode.DROPDOWN,
    ...
    async def async_step_choose_edit_entity(self, user_input=None):
        ...
        if user_input is not None:
            ...
            try:
                ...
                if 0 <= idx < len(self._controlled_entities):
                    ...
                    self._current_entity_config = copy.deepcopy(
                        self._controlled_entities[idx]
                    )
                    self._selected_entity_id = self._current_entity_config[
                        CONF_ENTITY_ID
                    ]
                    self._custom_state_ui = {}
                    return await self.async_step_configure_entity()
        ...
        options: list[selector.SelectOptionDict] = [
            selector.SelectOptionDict(
                value=str(idx), label=self._format_entity_label(entity)
            )
...
class PresenceBasedLightingOptionsFlowHandler(
    _EntityManagementMixin, config_entries.OptionsFlow
):
    ...
    def __init__(self, config_entry):
        ...
        self._base_data = {
            ...
            CONF_CLEARING_SENSORS_AUTO_DISCOVERED: config_entry.data.get(
                CONF_CLEARING_SENSORS_AUTO_DISCOVERED, False
            ),
            CONF_ACTIVATION_CONDITIONS: config_entry.data.get(
                CONF_ACTIVATION_CONDITIONS, []
            ),
            ...
            CONF_FILE_LOGGING_ENABLED: config_entry.data.get(
                CONF_FILE_LOGGING_ENABLED, DEFAULT_FILE_LOGGING_ENABLED
            ),
            CONF_OFF_DELAY: config_entry.data.get(CONF_OFF_DELAY, DEFAULT_OFF_DELAY),
            CONF_AUTO_REENABLE_PRESENCE_SENSORS: config_entry.data.get(
                CONF_AUTO_REENABLE_PRESENCE_SENSORS, []
            ),
            CONF_AUTO_REENABLE_VACANCY_THRESHOLD: config_entry.data.get(
                CONF_AUTO_REENABLE_VACANCY_THRESHOLD,
                DEFAULT_AUTO_REENABLE_VACANCY_THRESHOLD,
            ),
            CONF_AUTO_REENABLE_START_TIME: config_entry.data.get(
                CONF_AUTO_REENABLE_START_TIME, DEFAULT_AUTO_REENABLE_START_TIME
            ),
            CONF_AUTO_REENABLE_END_TIME: config_entry.data.get(
                CONF_AUTO_REENABLE_END_TIME, DEFAULT_AUTO_REENABLE_END_TIME
            ),
        }
        ...
        self._custom_state_ui: dict[str, str | None] = {}
        _LOGGER.debug(
            "OptionsFlow.__init__ complete. Loaded %d entities",
    ...
    def _cleanup_presence_switch(self, entity_id: str | None) -> None:
        ...
        if existing_entity_id:
            _LOGGER.debug(
                "Removing presence switch %s for entity %s",
                existing_entity_id,
                entity_id,
            )
                ...
                if not indices:
                    ...
                    for idx in sorted(set(indices), reverse=True):
                        if 0 <= idx < len(self._controlled_entities):
                            ...
                            _LOGGER.debug(
                                "Removed entity %s during delete flow", removed
                            )
    ...
    async def async_step_init(self, user_input=None):
        ...
        if user_input is not None:
            ...
            previous_clearing = list(
                self._base_data.get(CONF_CLEARING_SENSORS, []) or []
            )
            ...
            self._base_data[CONF_AUTO_REENABLE_PRESENCE_SENSORS] = user_input.get(
                CONF_AUTO_REENABLE_PRESENCE_SENSORS, []
            )
            self._base_data[CONF_AUTO_REENABLE_VACANCY_THRESHOLD] = user_input.get(
                CONF_AUTO_REENABLE_VACANCY_THRESHOLD,
                DEFAULT_AUTO_REENABLE_VACANCY_THRESHOLD,
            )
            self._base_data[CONF_AUTO_REENABLE_START_TIME] = user_input.get(
                CONF_AUTO_REENABLE_START_TIME, DEFAULT_AUTO_REENABLE_START_TIME
            )
            self._base_data[CONF_AUTO_REENABLE_END_TIME] = user_input.get(
                CONF_AUTO_REENABLE_END_TIME, DEFAULT_AUTO_REENABLE_END_TIME
            )
            self._selected_entity_id = None
            _LOGGER.debug(
    ...
    async def async_step_configure_entity(self, user_input=None):
        ...
        defaults = {
            ...
            CONF_CONTROL_LEASE_MODE: self._current_entity_config.get(
                CONF_CONTROL_LEASE_MODE,
                DEFAULT_CONTROL_LEASE_MODE,
            ),
            CONF_CONTROL_LEASE_BLOCKERS: self._current_entity_config.get(
                CONF_CONTROL_LEASE_BLOCKERS,
                list(DEFAULT_CONTROL_LEASE_BLOCKERS),
            ),
            CONF_CONTROL_LEASE_CORRECT_LATE_ON: self._current_entity_config.get(
                CONF_CONTROL_LEASE_CORRECT_LATE_ON,
                DEFAULT_CONTROL_LEASE_CORRECT_LATE_ON,
            ),
        }
