...
class ExternalOverrideManager:
    ...
    def clear(
        self, entity_id: str, reason: str = "cleared", notify: bool = True
    ) -> bool:
    ...
    def upsert_confirmed_batch(
        ...
        *,
        ...
        if (
            ...
            and record.source
            not in {SOURCE_HOMEKIT_BATCH, SOURCE_HOMEKIT_SINGLE, SOURCE_UNKNOWN}
    ...
    def _notify(self, entity_id: str) -> None:
        for listener in list(self._listeners.get(entity_id, {}).values()):
            try:
                ...
                _LOGGER.exception(
                    "Error notifying override listener for %s: %s", entity_id, err
                )
