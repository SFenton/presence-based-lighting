...
def is_real_last_changed_entity(
    entity_id: str | None, state: "State | None" = None
) -> bool:
    ...
    return entity_id.startswith("sensor.") and not entity_id.startswith(
        "sensor.binary_"
    )
...
def is_rlc_integration_available(hass: "HomeAssistant") -> bool:
    ...
    for state in hass.states.async_all():
        if (
            ...
            and ATTR_PREVIOUS_VALID_STATE in state.attributes
        ):
            return True
    return False
...
def get_rlc_sensors_for_entity(
    ...
    target_name = (
        target_entity_id.split(".", 1)[-1]
        if "." in target_entity_id
...
def get_matching_rlc_sensor_for_entity(
    ...
    if is_real_last_changed_entity(target_entity_id, target_state) and (
        ...
        or _effective_state_matches(target_state, valid_effective_states)
    ):
