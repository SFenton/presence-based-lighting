...
from .const import BATCH_MODE_ENFORCE
from .const import BATCH_MODE_OBSERVE
...
from .const import EVENT_HOMEKIT_STATE_CHANGE
...
class CommandBatchObserver:
    ...
    def __init__(
        self, hass: HomeAssistant, time_source: Callable[[], float] | None = None
    ) -> None:
        self.hass = hass
        ...
        self._confirm_callbacks: list[Callable[[CommandBatch, str | None], None]] = []
    ...
    def _attach_listener(self) -> None:
        ...
        try:
            ...
            _LOGGER.warning(
                "Could not listen for %s: %s", EVENT_HOMEKIT_STATE_CHANGE, err
            )
    ...
    def _detach_listener(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error removing %s listener: %s", EVENT_HOMEKIT_STATE_CHANGE, err
            )
        self._unsub = None
    ...
    async def _handle_homekit_event(self, event) -> None:
        ...
        try:
            ...
            _LOGGER.exception(
                "Error handling %s event: %s", EVENT_HOMEKIT_STATE_CHANGE, err
            )

    def note_command(
        self, entity_id: str, service: str, context_id: str
    ) -> CommandBatch | None:
        ...
        if newly_confirmed or (batch.confirmed and (entity_was_new or context_was_new)):
    ...
    def _open_batch_for(self, service: str, now: float) -> CommandBatch:
        ...
        if open_id is not None:
            ...
            if (
                ...
                and (now - existing.started_at) <= self._window_seconds
            ):
                return existing
    ...
    def pop_blocked_for_batch(
        ...
        for key, command in list(self._blocked_commands.items()):
            if command.context_id in context_ids and command.entity_id in entity_ids:
