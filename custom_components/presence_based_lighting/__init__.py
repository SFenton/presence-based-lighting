...
from .batch_observer import BlockedCommand
...
from .const import AUTOMATION_CONTROL_STATE_ACTIVE
from .const import AUTOMATION_CONTROL_STATE_OFF
from .const import AUTOMATION_CONTROL_STATE_ON
from .const import AUTOMATION_CONTROL_STATE_PAUSED
from .const import AUTOMATION_CONTROL_STATE_QUIETED
...
from .const import CONF_BATCH_MIN_DISTINCT_ENTITIES
from .const import CONF_BATCH_RETAIN_SECONDS
from .const import CONF_BATCH_WINDOW_MS
...
from .const import CONF_HOMEKIT_BATCH_MODE
...
from .const import ENABLE_FILE_LOGGING
from .const import EVENT_COMMAND_INTENT
...
from .entity_targeting import slugify_entity_id
from .external_override import get_external_override_manager
from .interceptor import ControlLeaseInterceptor
from .interceptor import is_interceptor_available
from .interceptor import PresenceLockInterceptor
from .ownership import get_ownership_manager
from .real_last_changed import ATTR_PREVIOUS_VALID_STATE
from .real_last_changed import get_effective_state
from .real_last_changed import is_entity_off
from .real_last_changed import is_entity_on
...
async def async_setup(hass: HomeAssistant, _config: dict) -> bool:
    ...
    await lease_manager.async_initialize()
    lease_manager.attach_enforcement_guard(ControlLeaseInterceptor(hass, lease_manager))
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    Version 12 -> 13: Add default-off control-lease safety settings.
    """
    _LOGGER.debug(
    ...
    if config_entry.version == 2:
        ...
        for entity_config in controlled_entities:
            ...
            if CONF_AUTOMATION_MODE not in updated_config:
                ...
                is_presence_lock = (
                    updated_config[CONF_AUTOMATION_MODE]
    ...
    if config_entry.version == 3:
        ...
        for entity_config in controlled_entities:
            ...
            if CONF_MANUAL_DISABLE_STATES not in updated_config:
                updated_config[CONF_MANUAL_DISABLE_STATES] = list(
    ...
    if config_entry.version == 12:
        ...
        _LOGGER.info(
            "Migration of entry %s from version 12 to 13 successful",
            config_entry.entry_id,
        )
...
def _get_exact_room_aod_clearing_sensor(
...
def _schedule_aod_clearing_autofill_retry(
...
def _schedule_rlc_sensor_migration_retry(
    hass: HomeAssistant, entry: ConfigEntry
) -> None:
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        _LOGGER.debug(
            "Creating coordinator for entry: %s with data: %s",
            entry.entry_id,
            entry.data,
        )
        ...
        _LOGGER.info(
            "Successfully set up Presence Based Lighting entry: %s", entry.entry_id
        )
        return True
    except Exception as err:
        _LOGGER.exception(
            "Failed to set up Presence Based Lighting entry %s: %s", entry.entry_id, err
        )
        return False
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        coordinator: PresenceBasedLightingCoordinator = hass.data[DOMAIN][
            entry.entry_id
        ]
        ...
        if unload_ok:
            ...
            _LOGGER.info(
                "Successfully unloaded Presence Based Lighting entry: %s",
                entry.entry_id,
            )
        else:
        ...
        _LOGGER.exception(
            "Error unloading Presence Based Lighting entry %s: %s", entry.entry_id, err
        )
        return False
...
async def async_reload_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    try:
        _LOGGER.info(
            "Reloading Presence Based Lighting entry via HA: %s", entry.entry_id
        )
        await hass.config_entries.async_reload(entry.entry_id)
        _LOGGER.info(
            "Successfully reloaded Presence Based Lighting entry: %s", entry.entry_id
        )
    except Exception as err:
        _LOGGER.exception(
            "Error reloading Presence Based Lighting entry %s: %s", entry.entry_id, err
        )
...
class PresenceBasedLightingCoordinator:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._control_lease_manager = get_control_lease_manager(hass)
        ...
        try:
            ...
            for idx, entity in enumerate(controlled_entities):
                ...
                if not entity_id:
                    _LOGGER.error(
                        "Entity at index %d is missing entity_id: %s", idx, entity
                    )
                    continue

                if entity_id in seen_entities:
                    _LOGGER.warning(
                        "Duplicate entity_id detected: %s (skipping duplicate)",
                        entity_id,
                    )
                    continue
                ...
                _LOGGER.debug(
                    "Configuring entity %d: %s with config: %s", idx, entity_id, entity
                )
                ...
                self._control_lease_manager.register_entity(
                    ...
                    blocker_entity_ids=entity.get(
                        CONF_CONTROL_LEASE_BLOCKERS,
                        DEFAULT_CONTROL_LEASE_BLOCKERS,
                    ),
                    correct_late_on=entity.get(
                        CONF_CONTROL_LEASE_CORRECT_LATE_ON,
                        DEFAULT_CONTROL_LEASE_CORRECT_LATE_ON,
                    ),
                    listener=self._handle_control_lease_changed,
                    blocker_callback=lambda entity_id=entity_id: (
                        self._control_lease_blockers(entity_id)
                    ),
                    baseline_callback=lambda entity_id=entity_id: (
                        self._control_lease_baseline_fingerprint(entity_id)
                    ),
                    completion_callback=self._compare_clear_control_lease_baseline,
                )

            _LOGGER.info(
                "Coordinator initialized with %d unique entities",
                len(self._entity_states),
            )
        except Exception as err:
            _LOGGER.exception(
                "Error initializing PresenceBasedLightingCoordinator: %s", err
            )
            raise
    ...
    def _actuation_generation_is_current(self, entity_state: dict) -> bool:
        ...
        return generation is None or generation == self._entity_work_generation(
            entity_state
        )
    ...
    def _actuation_is_currently_pending(self, entity_state: dict) -> bool:
        ...
        return (
            ...
            and (
                bool(actuation.get("dispatching")) or actuation.get("timer") is not None
            )
        )
    ...
    async def async_start(self) -> None:
        ...
        try:
            ...
            await self._control_lease_manager.async_initialize()
            ...
            self._using_interceptor = self._interceptor.has_presence_lock_interceptors

            if interceptors_registered:
                _LOGGER.info("Using hass-interceptor for pre-dispatch service handling")
            ...
            legacy_authority_sensors = self.entry.data.get(
            ...
            self._clearing_sensors = (
                set(clearing_sensors) if clearing_sensors else set(presence_sensors)
            )
            ...
            for entity_id, entity_state in self._entity_states.items():
                ...
                _LOGGER.debug(
                    "Checking RLC init for %s: tracking_entity=%s",
                    entity_id,
                    rlc_tracking_entity,
                )
                if rlc_tracking_entity:
                    self._rlc_to_entity[rlc_tracking_entity] = entity_id
                    rlc_state = self._get_valid_rlc_effective_state(
                        entity_id, entity_state
                    )
                    _LOGGER.debug(
                        "RLC state for %s from %s: %s",
                        entity_id,
                        rlc_tracking_entity,
                        rlc_state,
                    )
            ...
            _LOGGER.debug(
                "Setting up listeners for %d controlled entities: %s",
                len(controlled_ids),
                controlled_ids,
            )
            _LOGGER.debug(
                "Setting up listeners for %d presence sensors: %s",
                len(presence_sensors),
                presence_sensors,
            )
            _LOGGER.debug(
                "Setting up listeners for %d clearing sensors: %s",
            ...
            if controlled_ids:
                ...
                _LOGGER.debug(
                    "Registered state change listener for controlled entities"
                )
            ...
            if all_sensors:
                ...
                _LOGGER.debug(
                    "Registered state change listener for presence/clearing/vacancy authority sensors"
                )
            ...
            if activation_conditions:
                ...
                _LOGGER.debug(
                    "Registered state change listener for %d activation conditions: %s",
                    len(activation_conditions),
                    activation_conditions,
                )
            ...
            auto_reenable_sensors = self.entry.data.get(
                CONF_AUTO_REENABLE_PRESENCE_SENSORS, []
            )
            if auto_reenable_sensors:
                ...
                _LOGGER.debug(
                    "Registered state change listener for %d auto-reenable sensors: %s",
                    len(auto_reenable_sensors),
                    auto_reenable_sensors,
                )
            ...
            _LOGGER.info(
                "Coordinator started successfully with %d listeners",
                len(self._listeners),
            )
        except Exception as err:
            _LOGGER.exception(
                "Error starting PresenceBasedLightingCoordinator: %s", err
            )
            raise
    ...
    def async_stop(self) -> None:
        ...
        try:
            ...
            self._control_lease_manager.unregister_entry(self.entry.entry_id)
            ...
            _LOGGER.info(
                "Coordinator stopped successfully, removed %d listeners", listener_count
            )
        except Exception as err:
            _LOGGER.exception(
                "Error stopping PresenceBasedLightingCoordinator: %s", err
            )
    ...
    def _control_lease_active(self, entity_id: str) -> bool:
        return self._control_lease_manager.has_suppression(entity_id)

    def _control_lease_blockers(self, entity_id: str) -> list[str]:
        """Return stable acquire blockers from this entry's current base state."""
        entity_state = self._entity_states.get(entity_id)
        if entity_state is None:
            return ["entity_not_registered"]
        blockers: list[str] = []
        admitted_baseline = self._control_lease_baseline_fingerprint(entity_id)
        if not self._presence_switch_allows_entity(entity_state):
            blockers.append("presence_allowed_off")
        if (
            entity_state["state"] == EntityAutomationState.PAUSED
            and admitted_baseline is None
        ):
            blockers.append("automation_paused")
        if entity_state["state"] == EntityAutomationState.QUIETED:
            blockers.append("automation_quieted")
        if (
            self._override_manager.get(entity_id) is not None
            and admitted_baseline is None
        ):
            blockers.append("external_override_active")
        actuation = entity_state["actuation"]
        if actuation["status"] == ActuationStatus.PENDING and (
            bool(actuation.get("dispatching"))
            or int(actuation.get("attempts") or 0) > 0
        ):
            blockers.append("actuation_in_flight")
        return blockers

    def _control_lease_baseline_fingerprint(
        self,
        entity_id: str,
    ) -> BaselineSuppressionFingerprint | None:
        """Return an exact qualified asleep/manual-off suppression fingerprint."""
        entity_state = self._entity_states.get(entity_id)
        if entity_state is None:
            return None
        ...
        if (
            entity_state["state"] != EntityAutomationState.PAUSED
            or pause.get("source") != "external_override"
            or not pause.get("paused_at")
            or override is None
            or not override.is_paused
            or override.source
            not in {
            ...
            return None
        return BaselineSuppressionFingerprint(
            entry_id=self.entry.entry_id,
            override_source=override.source,
            override_policy=override.policy,
            override_created_at=override.created_at,
            pause_source=str(pause["source"]),
            pause_paused_at=str(pause["paused_at"]),
        )

    def _compare_clear_control_lease_baseline(
        self,
        entity_id: str,
        fingerprints: tuple[BaselineSuppressionFingerprint, ...],
    ) -> str:
        """Clear only the unchanged asleep suppression admitted at acquire."""
        if not fingerprints:
            return "baseline_none"
        override = self._override_manager.get(entity_id)
        first = fingerprints[0]
        if any(
            fingerprint.override_source != first.override_source
            or fingerprint.override_policy != first.override_policy
            or fingerprint.override_created_at != first.override_created_at
            for fingerprint in fingerprints[1:]
        ):
            return "baseline_preserved_fingerprint_ambiguous"
        if (
            override is None
            or override.source != first.override_source
            or override.policy != first.override_policy
            or override.created_at != first.override_created_at
        ):
            return "baseline_preserved_override_changed"

        domain_data = self.hass.data.get(DOMAIN, {})
        for fingerprint in fingerprints:
            controller = domain_data.get(fingerprint.entry_id)
            if (
                not isinstance(controller, PresenceBasedLightingCoordinator)
                or entity_id not in controller._entity_states
            ):
                return "baseline_preserved_entry_missing"
            entity_state = controller._entity_states[entity_id]
            pause = entity_state.get("pause") or {}
            if (
                entity_state["state"] != EntityAutomationState.PAUSED
                or pause.get("source") != fingerprint.pause_source
                or pause.get("paused_at") != fingerprint.pause_paused_at
            ):
                return "baseline_preserved_pause_changed"

        self._clear_external_override(
            entity_id,
            "completed control lease compare-cleared admitted asleep suppression",
        )
        return "baseline_compare_cleared"

    def _handle_control_lease_changed(
        self,
        entity_id: str,
        transition: dict[str, Any],
    ) -> None:
        """Apply the lease overlay without replacing the base automation state."""
        entity_state = self._entity_states.get(entity_id)
        if entity_state is None:
            return
        if self._control_lease_active(entity_id):
            self._bump_entity_work_generation(entity_state, "control lease acquired")
            self._cancel_entity_timer(entity_state)
            self._cancel_entity_actuation(entity_state, "control lease acquired")
        ...
        if (
            action == "released"
            and transition.get("outcome") == "cancelled"
            and cause == CONTROL_LEASE_RELEASE_CAUSE_EXTERNAL_OFF
        ):
            if self._control_lease_baseline_fingerprint(entity_id) is None:
                self._apply_external_policy(
                    entity_id,
                    entity_state,
                    None,
                    "leased light targets switched off externally",
                )
            self._publish_ownership(entity_id, entity_state)
            self._notify_switch(entity_id)
            return
        self._publish_ownership(entity_id, entity_state)
        self._notify_switch(entity_id)
        if isinstance(action, str) and action.startswith("would_"):
            return
        if action not in {"released", "revoked"}:
            return
        if cause in {
            "context_enforcement_unavailable",
            ...
            self.set_automation_paused(
                entity_id,
                True,
                reason=f"control lease safety break: {cause}",
        ...
        if cause in {
            ...
            "bulk_off",
            "baseline_suppression_changed",
            "controlled_entity_off",
            "presence_allowed_off",
        ...
        async def _settle_after_lease() -> None:
            if cause == "recovery_timeout" and self._manual_disable_state_matches(
                entity_state,
                self._get_trusted_effective_controlled_state(entity_state),
            ):
                self._apply_external_policy(
                    entity_id,
                    entity_state,
                    None,
                    "restored control lease timed out while entity was manually off",
                )
                return
        ...
        try:
            ...
            if create_task is not None:
                create_task(_settle_after_lease())
            else:
                asyncio.create_task(_settle_after_lease())
        except RuntimeError:
            _LOGGER.debug("No running loop available to settle control lease")
    ...
    def get_entity_control_state(self, entity_id: str) -> dict:
        ...
        pause = entity_state.get("pause") or {}
        override = self._override_manager.get(entity_id)
        base_suppressed = entity_state["state"] in (
            EntityAutomationState.PAUSED,
            EntityAutomationState.QUIETED,
        )
        lease_suppressed = self._control_lease_active(entity_id)
        suppressed = base_suppressed or lease_suppressed
        suppression_kind = (
            "leased"
            if lease_suppressed
        ...
        attributes = {
            ...
            "quieted_max_age_reached_at": (
                override.max_age_reached_at if override else None
            ),
            "unknown_source_count": self._override_manager.unknown_source_count(
                entity_id
            ),
            "homekit_batch_mode": self._batch_observer.mode,
        }
        attributes.update(self._control_lease_manager.attributes_for(entity_id))
        return attributes
    ...
    def _valid_effective_states_for_entity(self, entity_state: dict) -> set[str]:
        ...
        states.update(
            config.get(CONF_MANUAL_DISABLE_STATES, DEFAULT_MANUAL_DISABLE_STATES) or []
        )
        return {state for state in states if state is not None}

    def _get_valid_rlc_effective_state(
    ...
    def _build_pause_metadata(
        self, entity_id: str, entity_state: dict, reason: str, source: str
    ) -> dict:
        return {
            "entity_id": entity_id,
            "source": source,
            ...
            "controlled_state": self._get_trusted_effective_controlled_state(
                entity_state
            ),
    ...
    def _should_restore_pause(
        self, entity_id: str, entity_state: dict, metadata: dict
    ) -> bool:
    ...
    async def _recover_taskless_pending_actuation(
        ...
        if actuation[
            "status"
        ] != ActuationStatus.PENDING or self._actuation_is_currently_pending(
            entity_state
        ):
            return False
    ...
    def _intent_for_service(
        ...
        if config[service_key] == NO_ACTION:
        ...
        elif not presence_lock_override and not self._presence_switch_allows_entity(
            entity_state
        ):
            authority = False
        ...
        elif self._control_lease_manager.has_brightness_authority(entity_id):
            authority = False
            intent_reason = IntentReason.CONTROL_LEASE
        elif (
            ...
            and self._ownership_manager.other_entry_wants_on(
                self.entry.entry_id, entity_id
            )
        ):
            authority = False
    ...
    async def _apply_intent(
        self, entity_id: str, entity_state: dict, intent: dict
    ) -> bool:
        ...
        if (
            ...
            if intent["desired"] == DesiredState.CLEARED and entity_state[
                "state"
            ] not in (
                EntityAutomationState.SETTLING_OFF,
                EntityAutomationState.PAUSED,
    ...
    async def _begin_entity_actuation(
        self, entity_id: str, entity_state: dict, intent: dict
    ) -> None:
        config = entity_state["config"]
        ...
        generation = self._bump_entity_work_generation(
            entity_state, "new actuation intent"
        )
        actuation = entity_state["actuation"]
        actuation.update(
            {
        ...
        if (
            ...
            self._set_entity_state(
                ...
                entity_state,
                EntityAutomationState.SETTLING_ON,
                "actuating detected intent",
            )

        current_state = self.hass.states.get(entity_id)
        if (
            current_state
            and current_state.state == target_state
            and not force_service_call
        ):
    ...
    def _actuation_target_is_still_valid(
        self, entity_id: str, entity_state: dict
    ) -> bool:
        intent = entity_state["intent"]
        actuation = entity_state["actuation"]
        ...
        if (
            ...
            return False
        if self._control_lease_manager.has_brightness_authority(entity_id):
            return False
        if not presence_lock_override and not self._presence_switch_allows_entity(
            entity_state
        ):
            return False
    ...
    def _schedule_actuation_timer(
        self, entity_id: str, entity_state: dict, delay: float, retry: bool
    ) -> None:
        self._cancel_actuation_timer(entity_state)
        generation = entity_state["actuation"].get("generation")
        if retry:
            task = asyncio.create_task(
                self._execute_actuation_retry_timer(
                    entity_id, entity_state, delay, generation
                )
            )
        else:
            task = asyncio.create_task(
                self._execute_actuation_confirmation_timer(
                    entity_id, entity_state, delay, generation
                )
            )
    ...
    async def _execute_actuation_confirmation_timer(
        ...
        try:
            ...
            if not self._entity_work_generation_matches(
                entity_id, entity_state, generation, "actuation confirmation"
            ):
                return
            actuation = entity_state["actuation"]
            if actuation["status"] != ActuationStatus.PENDING:
            ...
            if observed == actuation["target_state"]:
                ...
                await self._retry_or_fail_entity_actuation(
                    entity_id, entity_state, observed
                )
        except asyncio.CancelledError:
            _LOGGER.debug("[%s] Actuation confirmation timer cancelled", entity_id)
        except Exception as err:
            _LOGGER.exception(
                "[%s] Error in actuation confirmation timer: %s", entity_id, err
            )
        finally:
    ...
    async def _execute_actuation_retry_timer(
        ...
        try:
            await asyncio.sleep(delay)
            await self._send_entity_actuation_attempt(
                entity_id, entity_state, generation
            )
        except asyncio.CancelledError:
    ...
    async def _send_entity_actuation_attempt(
        ...
        if not self._entity_work_generation_matches(
            entity_id, entity_state, generation, "actuation attempt"
        ):
            return
        if actuation["status"] != ActuationStatus.PENDING:
            return
        ...
        observed = current_state.state if current_state else None
        actuation["last_observed_state"] = observed
        if observed == actuation["target_state"] and not actuation.get(
            "force_service_call"
        ):
            actuation["next_retry_delay"] = None
            self._schedule_actuation_timer(
                entity_id, entity_state, _ACTUATION_CONFIRMATION_SECONDS, retry=False
            )
            return
        ...
        if not self._entity_work_generation_matches(
            entity_id, entity_state, generation, "actuation service call"
        ):
            return
        if not self._actuation_target_is_still_valid(entity_id, entity_state):
            self._cancel_entity_actuation(entity_state, "intent no longer valid")
        ...
        if self._entity_work_generation_matches(
            entity_id, entity_state, generation, "actuation confirmation scheduling"
        ):
            self._schedule_actuation_timer(
                entity_id,
                entity_state,
    ...
    async def _handle_external_change_matching_actuation_target(
        ...
        if actuation["status"] != ActuationStatus.FAILED:
            return False
        if (
            effective_new_state is None
            or effective_new_state != actuation["target_state"]
        ):
            return False
    ...
    def _confirm_entity_actuation(
        ...
        _LOGGER.debug(
            "[%s] Actuation confirmed target=%s", entity_id, actuation["target_state"]
        )
        if (
            service_key == CONF_PRESENCE_CLEARED_SERVICE
            and entity_state["state"] != EntityAutomationState.PAUSED
            ...
            self._set_entity_state(
                ...
                entity_state,
                EntityAutomationState.IDLE,
                "actuation confirmed cleared",
            )
        elif (
            service_key == CONF_PRESENCE_DETECTED_SERVICE
            and entity_state["state"] == EntityAutomationState.SETTLING_ON
        ):
            self._set_entity_state(
                entity_id,
                entity_state,
                EntityAutomationState.OCCUPIED,
                "actuation confirmed detected",
            )
        self._notify_switch(entity_id)
    ...
    async def async_set_automation_control_state(
        ...
        if control_state == AUTOMATION_CONTROL_STATE_QUIETED:
            ...
            self._override_manager.set_override(
                ...
                rearm_armed_by=(
                    "already clear at admin request" if rearm_latched else None
                ),
    ...
    def _manual_disable_state_matches(
    ...
    def _presence_switch_entity_ids(
        self, entity_id: str, entity_state: dict
    ) -> set[str]:
        room_name = self.entry.data.get(CONF_ROOM_NAME, "")
    ...
    def set_automation_paused(
        ...
        """
        entity_state = self._entity_states[entity_id]
        if paused and self._control_lease_manager.has_candidate(entity_id):
            self._control_lease_manager.break_nowait(
                entity_id,
                cause="admin_conflict",
                direction="off",
        ...
        if is_paused == paused:
            if paused:
                entity_state["pause"] = self._build_pause_metadata(
                    entity_id, entity_state, reason, source
                )
                self._schedule_paused_state_save()
            return
        ...
        if paused:
            ...
            self._set_entity_state(
                entity_id, entity_state, EntityAutomationState.PAUSED, "manual control"
            )
        else:
    ...
    def _get_paused_persistence_path(self) -> Path:
        ...
        return (
            ...
            / f"pbl_paused_{self.entry.entry_id}.json"
        )
    ...
    async def _save_paused_state(self) -> None:
        ...
        async with self._paused_state_save_lock:
            try:
                ...
                if (
                    not paused_entities
                    and not quieted_entities
                    and not override_entities
                ):
    ...
    async def _load_paused_state(self) -> None:
        ...
        try:
            ...
            for entity_id in paused_entities:
                entity_state = self._entity_states.get(entity_id)
                if not entity_state or not self._entity_respects_presence_allowed(
                    entity_state
                ):
                    continue
    ...
    async def _handle_service_call(self, event: Event) -> None:
        try:
            ...
            if (
                ...
                and service == "turn_on"
                and self._control_lease_manager.service_data_has_brightness_authority(
                    service_data
                )
                and self._control_lease_manager.has_any_candidate()
            ):
                authority_call = ServiceCall(
                    self.hass,
                    event.data["domain"],
                    service,
                    service_data,
                ...
                authority_targets = await async_extract_service_target_entity_ids(
                    authority_call,
                )
                await self._control_lease_manager.async_break_for_brightness_targets(
                    authority_targets,
            ...
            for root_entity_id in self._control_lease_manager.manual_authority_roots(
                event.context
            ):
                if (
                    root_entity_id in self._entity_states
                    and root_entity_id not in expanded_entities
                ):
                    expanded_entities.append(root_entity_id)
            ...
            for entity_id in expanded_entities:
                ...
                if manual_authority_action is not None:
                    if manual_authority_action == "apply":
                        self._apply_external_policy(
                            entity_id,
                            entity_state,
                            event.context,
                ...
                if origin != CommandOrigin.EXTERNAL:
                    continue
                if expected_target_state == config.get(
                    CONF_PRESENCE_CLEARED_STATE
                ) and self._control_lease_manager.has_candidate(entity_id):
                    resolved = self._resolve_external_policy(
                        entity_id,
                        entity_state,
                        event.context,
                    )
                    await self._control_lease_manager.async_break(
                        ...
                        cause=(
                            ...
                            else "manual_group_off"
                        ),
                        direction="off",
                        context_classification=(
                            self._control_lease_manager.classify_external_context(
                                event.context
                            )
                        ),
                    )
    ...
    async def _handle_controlled_entity_change(self, event: Event) -> None:
        try:
            ...
            if rlc_tracking_entity:
                ...
                if rlc_state is None:
                    ...
                    if effective_new_state == last_effective:
                        if origin == CommandOrigin.OWN:
                            await self._handle_actuation_feedback(
                                entity_id, entity_state, new_state.state
                            )
                        ...
                        elif origin == CommandOrigin.CONTROL_LEASE:
                            self._control_lease_manager.note_controlled_state(
                                entity_id,
                                effective_new_state,
                                new_state.context,
                            )
                ...
                if _is_untrusted_state_value(old_state) or _is_untrusted_state_value(
                    new_state
                ):
                    _LOGGER.debug(
            ...
            if origin == CommandOrigin.CONTROL_LEASE:
                self._control_lease_manager.note_controlled_state(
                    entity_id,
                    effective_new_state,
                    new_state.context,
            ...
            if self._control_lease_manager.has_candidate(
                entity_id
            ) and self._manual_disable_state_matches(
                entity_state,
                effective_new_state,
            ):
                await self._control_lease_manager.async_break(
                    entity_id,
                    cause="controlled_entity_off",
                    direction="off",
                    context_classification=(
                        self._control_lease_manager.classify_external_context(
                            new_state.context
            ...
            if self._control_lease_active(entity_id):
                if not self._manual_disable_state_matches(
                    entity_state,
                    effective_new_state,
                ):
                    self._notify_switch(entity_id)
            ...
            _LOGGER.exception(
                "Error handling controlled entity change for %s: %s",
    ...
    async def _handle_rlc_tracking_change(self, event: Event) -> None:
        ...
        try:
            ...
            old_effective = (
                old_state.attributes.get(ATTR_PREVIOUS_VALID_STATE)
                if old_state
                else None
            )
            ...
            entity_state = self._entity_states[entity_id]
            if new_effective not in self._valid_effective_states_for_entity(
                entity_state
            ):
                _LOGGER.warning(
            ...
            if origin == CommandOrigin.OWN:
                await self._handle_actuation_feedback(
                    entity_id, entity_state, new_effective
                )
                return
            if origin == CommandOrigin.SIBLING:
                await self._handle_sibling_controlled_change(
    ...
    async def _process_external_controlled_change(
        ...
        if not is_bulk:
            if self._presence_lock_should_yield_to_manual_override(
                entity_state, effective_new_state
            ):
                if self._manual_disable_state_matches(
                    entity_state, effective_new_state
                ):
                    self._apply_external_policy(
                        entity_id,
            ...
            if await self._check_and_apply_presence_lock(
                entity_state, effective_new_state
            ):
        ...
        should_pause = self._should_external_change_pause(
            entity_id, cfg, effective_new_state
        )
    ...
    async def _handle_external_action(
        ...
        *,
        ...
        if (
            not is_bulk
            and target_state
            and not (self._using_interceptor and interceptor_handles_direct_target)
    ...
    def _hold_allowed_presence_lock_clear(
        ...
        if policy == EXTERNAL_POLICY_REARM_AFTER_CLEAR and self._can_clear_room():
    ...
    def _get_trusted_effective_controlled_state(self, entity_state: dict) -> str | None:
        ...
        rlc_tracking_entity = cfg.get(CONF_RLC_TRACKING_ENTITY)
        if rlc_tracking_entity:
            return self._get_valid_rlc_effective_state(
                cfg[CONF_ENTITY_ID], entity_state
            )
    ...
    async def _check_and_apply_presence_lock(
        ...
        if (
            new_state == cfg[CONF_PRESENCE_DETECTED_STATE]
            and require_occ
            and not self._is_any_occupied()
        ):
            _LOGGER.debug(
                "Presence lock (fallback): reverting %s to cleared state (room is empty)",
                entity_id,
            )
        ...
        try:
            ...
            if is_rlc:
                ...
                if old_effective == new_effective:
                    _LOGGER.debug(
                        "RLC sensor %s timestamp changed but previous_valid_state unchanged (%s)",
                        entity_id,
                        new_effective,
                    )
                    return
                ...
                _LOGGER.debug(
                    "RLC sensor %s previous_valid_state changed: %s -> %s",
                    entity_id,
                    old_effective,
                    new_effective,
                )
                ...
                _LOGGER.debug(
                    "Presence change detected on %s: %s -> %s",
                    entity_id,
                    old_state.state,
                    new_state.state,
                )
                    ...
                    if cur in (
                        ...
                        if self._are_activation_conditions_met():
                            ...
                            self._set_entity_state(
                                ...
                                es,
                                EntityAutomationState.PENDING_ACTIVATION,
                                "presence detected, conditions not met",
                            )
                    ...
                    if all_clear:
                        ...
                        for eid, es in self._entity_states.items():
                            ...
                            if cur == EntityAutomationState.OCCUPIED:
                                ...
                                _LOGGER.debug(
                                    "%s: clear conditions met while WAITING_FOR_CLEAR, turning off",
                                    eid,
                                )
                                self._cancel_entity_timer(es)
                                await self._apply_service_intent(
    ...
    async def _handle_activation_condition_change(self, event: Event) -> None:
        ...
        try:
            ...
            if not self._entry_is_active():
                for eid, es in self._entity_states.items():
                    if self._control_lease_active(eid):
            ...
            for eid, es in self._entity_states.items():
                ...
                if (
                    ...
                    _LOGGER.debug(
                        "Activation conditions now met – transitioning %s from PENDING to OCCUPIED",
                        eid,
                    )
                    self._cancel_entity_timer(es)
                    self._set_entity_state(
                    ...
                    await self._apply_service_intent(
                        eid, es, CONF_PRESENCE_DETECTED_SERVICE, IntentReason.CONDITIONS
                    )
    ...
    async def _apply_presence_action(self, service_key: str) -> None:
        ...
        """
        _LOGGER.debug(
            "Applying presence action %s to %d entities: %s",
            service_key,
            len(self._entity_states),
            list(self._entity_states.keys()),
        )
        ...
        for entity_id, entity_state in self._entity_states.items():
            ...
            await self._apply_service_intent(
                entity_id, entity_state, service_key, reason
            )

    async def _apply_action_to_entity(
        self, entity_state: dict, service_key: str
    ) -> None:
    ...
    def _should_follow_presence(self, entity_state: dict) -> bool:
        ...
        return (
            ...
            and not self._control_lease_active(entity_id)
            and entity_state["state"]
            not in (
                EntityAutomationState.PAUSED,
                EntityAutomationState.QUIETED,
            )
    ...
    def _classify_command_context(
        ...
        *,
        ...
        if self._control_lease_manager.is_manual_authority_context(
            entity_id,
            context,
        ):
        ...
        if context_id in context_ids and (
    ...
    def _configure_batch_observer(self) -> None:
        ...
        self._batch_observer.configure_entry(
            ...
            retain_seconds=data.get(
                CONF_BATCH_RETAIN_SECONDS, DEFAULT_BATCH_RETAIN_SECONDS
            ),
    ...
    def _exit_quieted(self, entity_id: str, entity_state: dict, reason: str) -> None:
        ...
        entity_state["pause"] = None
        self._set_entity_state(
            entity_id, entity_state, EntityAutomationState.IDLE, reason
        )
        self._notify_switch(entity_id)
        self._schedule_paused_state_save()
    ...
    def _handle_batch_confirmed(
        ...
        for entity_id in affected_entities:
            ...
            self._control_lease_manager.break_nowait(
                ...
                cause="bulk_off",
                direction="off",
                context_classification="homekit_batch",
            )
            ...
            rearm_latched = (
                policy == EXTERNAL_POLICY_REARM_AFTER_CLEAR and self._can_clear_room()
            )
        ...
        if clearing:
            ...
            if not result:
                ...
                for sensor in clearing:
                    ...
                    if effective != "off":
                        _LOGGER.debug(
                            "Clearing sensor %s not clear: effective_state=%s",
                            sensor,
                            effective,
                        )
            return result
        ...
        if presence:
            ...
            if not result:
                for sensor in presence:
                    ...
                    if effective != "off":
                        _LOGGER.debug(
                            "Presence sensor (as clearing) %s not clear: effective_state=%s",
                            sensor,
                            effective,
                        )
            return result
    ...
    async def _start_entity_off_timer(self, entity_id: str, entity_state: dict) -> None:
        ...
        self._set_entity_state(
            ...
            entity_state,
            EntityAutomationState.CLEARING,
            f"off-timer started ({delay}s)",
        )
        task = asyncio.create_task(
            self._execute_entity_off_timer(entity_id, entity_state, delay, generation)
        )
    ...
    async def _execute_entity_off_timer(
        ...
        try:
            _LOGGER.debug(
                "[%s] Off timer sleeping %ds (state: CLEARING)", entity_id, delay
            )
            while True:
                await asyncio.sleep(delay)
                if not self._entity_work_generation_matches(
                    entity_id, entity_state, generation, "off timer"
                ):
                    return
                ...
                if self._can_clear_room():
                    if self._cleared_intent_blocked_by_presence(entity_state):
                        ...
                        continue
                    _LOGGER.debug(
                        "[%s] Timer fired, clear conditions met → cleared intent",
                        entity_id,
                    )
                    await self._apply_service_intent(
                        entity_id,
    ...
    def _set_entity_state(
        ...
        entity_id: str,
        entity_state: dict,
        new_state: EntityAutomationState,
        reason: str = "",
    ) -> None:
    ...
    def _entity_may_enforce_presence_lock(self, entity_id: str) -> bool:
        ...
        if not self._presence_lock_respects_manual_override(entity_state):
            return True
        return self._presence_switch_allows_entity(entity_state) and entity_state[
            "state"
        ] not in (
            EntityAutomationState.PAUSED,
            EntityAutomationState.QUIETED,
    ...
    def _should_external_change_pause(
        ...
        if CONF_MANUAL_DISABLE_STATES in cfg:
            ...
            should_pause = (
                effective_new_state is not None
                and effective_new_state in manual_disable_states
            )
            _LOGGER.debug(
        ...
        if occupied and conditions_met:
            if cur not in (
                ...
                self._set_entity_state(
                    ...
                    entity_state,
                    EntityAutomationState.OCCUPIED,
                    "reconcile: occupied + conditions met",
                )
                await self._apply_service_intent(
                    entity_id,
                    entity_state,
                ...
                current_ha_state = self.hass.states.get(
                    entity_state["config"][CONF_ENTITY_ID]
                )
                detected_state = entity_state["config"].get(
            ...
            if cur in (
                ...
                self._set_entity_state(
                    ...
                    entity_state,
                    EntityAutomationState.PENDING_ACTIVATION,
                    "reconcile: occupied, conditions not met",
                )
            else:
                self._publish_ownership(entity_id, entity_state)
        else:
            ...
            if cur in (
                ...
                if (
                    ...
                    self._set_entity_state(
                        ...
                        entity_state,
                        EntityAutomationState.OCCUPIED,
                        "reconcile: light on but room empty",
                    )
                    await self._start_entity_off_timer(entity_id, entity_state)
    ...
    async def _periodic_reconciliation(self, _now: datetime) -> None:
        ...
        try:
            now = dt_util.utcnow()
            await self._control_lease_manager.async_expire_due()
            ...
            for entity_id, es in self._entity_states.items():
                ...
                if cur == EntityAutomationState.WAITING_FOR_CLEAR:
                    if self._can_clear_room():
                        ...
                        if (
                            ...
                            if (
                                self._is_any_occupied()
                                or not self._are_clearing_sensors_clear()
                            ):
                                if self._entry_is_active():
                                    _LOGGER.info(
                                    ...
                                    self._set_entity_state(
                                        ...
                                        es,
                                        EntityAutomationState.OCCUPIED,
                                        "reconciliation: safety timeout but still occupied",
                                    )
                                    await self._apply_service_intent(
                                        entity_id,
                                        es,
                ...
                # CLEARING but timer somehow lost
                    ...
                    cur == EntityAutomationState.CLEARING
                    and es.get("off_timer") is None
                ):
                    _LOGGER.warning(
    ...
    def _get_tracking_persistence_path(self) -> Path:
        ...
        return (
            Path(self.hass.config.path(".storage"))
            / f"pbl_tracking_{self.entry.entry_id}.json"
        )
    ...
    async def _load_tracking_state(self) -> bool:
        ...
        try:
            ...
            if data.get("last_presence_change"):
                tracking["last_presence_change"] = datetime.fromisoformat(
                    data["last_presence_change"]
                )
    ...
    def get_auto_reenable_tracking_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "start_time": (
                str(self._auto_reenable_start_time)
                if self._auto_reenable_start_time
                else None
            ),
            "end_time": (
                str(self._auto_reenable_end_time)
                if self._auto_reenable_end_time
                else None
            ),
        }
        ...
        if tracking["is_tracking"] and tracking["window_start"]:
            ...
            if tracking["was_occupied"] and tracking["last_presence_change"]:
                current_occupied_seconds += (
                    now - tracking["last_presence_change"]
                ).total_seconds()
    ...
    async def _evaluate_and_apply_auto_reenable(self) -> None:
        ...
        if not tracking["is_tracking"]:
            _LOGGER.debug(
                "Auto re-enable evaluation skipped for %s: not tracking", room_name
            )
            return
    ...
    async def _reenable_presence_lighting(self) -> None:
        ...
        for entity_id, entity_state in self._entity_states.items():
            ...
            if not entity_state["presence_allowed"]:
                _LOGGER.info(
                    "Re-enabling presence lighting for %s in %s", entity_id, room_name
                )
                self._set_presence_allowed_value(entity_id, entity_state, True)
    ...
    async def _check_auto_reenable_startup(self) -> None:
        ...
        start_dt = dt_util.as_utc(
            datetime.combine(today, self._auto_reenable_start_time)
        )
        ...
        if tracking["window_start"]:
            ...
            # If we're still in the window, continue tracking
                ...
                _LOGGER.debug("Stale tracking state for %s, clearing", room_name)
                await self._clear_tracking_state()
